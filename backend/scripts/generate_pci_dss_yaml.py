"""Generates framework_mapping/pci_dss_v4.yaml (ADR-0027, extended ADR-0029).

Extends the original Section-level transcription (ADR-0027) down to the
real leaf-level "Defined Approach Requirement" granularity that ADR-0027
disclosed as future work. PCI DSS v4.0.1 is three levels deep:
Requirement (12) -> Section (63) -> Defined Approach Requirement (249,
confirmed by direct count during this transcription -- both the original
~205 estimate and a later ~250 guess made before this level was fully
transcribed were rough approximations; 249 is the real, counted total).

Copyright status is unchanged from ADR-0027: PCI DSS v4.0.1 is
copyrighted, all-rights-reserved content (the source PDF's own page 1
states "(c)2006 - 2024 PCI Security Standards Council, LLC. All Rights
Reserved"), despite being freely downloadable. Per that same disclosed
scope, only the bolded "Defined Approach Requirements" statement text is
transcribed here -- never the accompanying Testing Procedures, Purpose,
Good Practice, Examples, Definitions, Customized Approach Objective, or
Applicability Notes elaboration, which is the substantial, copyrightable
narrative content.

Structural change from ADR-0027: each Section (e.g. "1.1") is now a real
Objective (using the Section's own statement as Objective.title, e.g.
"1.1 Processes and mechanisms for installing and maintaining network
security controls are defined and understood."), and each Defined
Approach Requirement (e.g. "1.1.1") is a Practice. This mirrors how NERC
CIP's own Objectives carry a real requirement number and title rather
than a placeholder.

Source, verified directly: the same "Payment Card Industry Data Security
Standard: Requirements and Testing Procedures, v4.0.1" PDF (June 2024,
397 pages) used for the Section-level pass -- re-fetched from a public
institutional mirror and re-verified via pypdf (genuine title page, the
exact copyright notice quoted above, 397 real pages) before any leaf
text was extracted, the same "verify the actual content, don't trust
the URL" discipline ADR-0025/ADR-0026/ADR-0027 established. Every leaf
statement was extracted programmatically from the PDF's own "Defined
Approach Requirements" column (never the adjacent Testing
Procedures/Purpose/Good Practice/Definitions/Customized Approach
Objective/Applicability Notes columns), then spot-checked against the
raw extracted text per Requirement to catch and correct extraction
artifacts (mid-sentence line-wrap hyphens, a table layout that merged a
short requirement and its testing procedure onto one text line for a
few short leaf items, and a few leaf statements that PDF text order
placed a cross-referenced requirement number's start-of-line position
where a hand-off to the next leaf item was expected) -- all six such
corrections were verified against the raw page text before being fixed,
not guessed. All 249 leaf items were cross-checked against the
document's own N.N.N-pattern numbering.
"""
...
# (short_code, full_name, purpose, [(section_id, section_statement, [(leaf_id, leaf_text), ...]), ...])
REQUIREMENTS: list[tuple[str, str, str, list[tuple[str, str, list[tuple[str, str]]]]]] = [
    (
        "REQ-01",
    ...
    (
        ...
        'Actors with bad intentions can use security vulnerabilities to gain privileged access to systems. All system components must have all appropriate software patches to protect against the exploitation and compromise of account data by malicious individuals and malicious software.',
        [
            (
                '6.1',
        ...
        [
            ...
            (
                ...
                [
                    ...
                    ('11.3.1.1', 'All other applicable vulnerabilities (those not ranked as high-risk vulnerabilities or critical vulnerabilities according to the entity’s vulnerability risk rankings defined at Requirement 6.3.1) are managed as follows: Addressed based on the risk defined in the entity’s targeted risk analysis, which is performed according to all elements specified in Requirement 12.3.1. Rescans are conducted as needed.'),
...
def build_domain(
    ...
    return {
        ...
        "objectives": [
            {
                "number": number,
                "title": f"{section_id} {section_statement}",
                "purpose": "",
                "practices": [
                    {"id": leaf_id, "text": leaf_text, "applicability": ""}
                    for leaf_id, leaf_text in leaves
                ],
            }
            for number, (section_id, section_statement, leaves) in enumerate(sections, start=1)
        ],
        "source_version": "",
        "source_url": "",
    }
...
def main() -> None:
    framework = {
        ...
        "total_practices_in_source": 249,
        "scoring_model": "coverage",
        "mil_levels": [],
        "scoring_note": (
            "PCI DSS has no native maturity-level concept, like NIST CSF 2.0/NERC CIP/ISO "
            "27001/CIS Controls/SOC 2 — Practice.mil is always None here. Scores are the "
            "project-defined coverage measure via services/scoring_service.py's "
            "compute_domain_coverage. As of ADR-0029, Practice.text is real, verified "
            "leaf-level 'Defined Approach Requirement' statement text (e.g. '3.5.1 PAN is "
            "rendered unreadable anywhere it is stored...'), extended down from ADR-0027's "
            "original Section-level (N.N) transcription — PCI DSS v4.0.1 is copyrighted, "
            "all-rights-reserved content (confirmed directly: the source PDF's own page 1 "
            "states this explicitly), despite being freely downloadable, the same "
            "distinction ADR-0026 established for SOC 2; only the bolded requirement "
            "statement is transcribed, never the accompanying Testing Procedures/Purpose/"
            "Good Practice/Examples/Definitions/Customized Approach Objective/Applicability "
            "Notes elaboration. PCI DSS is uniquely THREE levels deep (Requirement -> "
            "Section -> Defined Approach Requirement, 249 real leaf-level items) — no other "
            "framework in this project has this depth. Each Section (63 total) is modeled "
            "as an Objective whose title carries the Section's own real number and "
            "statement (e.g. '3.5 Primary account number (PAN) is secured wherever it is "
            "stored.'), mirroring how NERC CIP's own Objectives carry a real requirement "
            "number and title; each Defined Approach Requirement is a Practice under that "
            "Objective. Practice.applicability is always empty here — no per-leaf-item "
            "applicability-scope concept was verified in the source (PCI DSS's SAQ-type "
            "scoping operates at a different level not captured per-item), unlike NERC "
            "CIP's impact tiers, CIS Controls' Implementation Groups, or SOC 2's Common/"
            "Additional-category distinction."
        ),
        "domains": [
    ...
    total_sections = sum(len(d["objectives"]) for d in framework["domains"])
    total_practices = sum(
    ...
    print(f"Sections encoded: {total_sections} of 63")
    print(f"Defined Approach Requirements encoded: {total_practices} of {framework['total_practices_in_source']}")
    assert total_sections == 63, (
        "Transcribed Section count does not match PCI DSS v4.0.1's own stated total of 63 "
        "Sections across its 12 Requirements — check REQUIREMENTS for a missing or "
        "duplicated Section before shipping this data."
    )
    assert total_practices == framework["total_practices_in_source"], (
        "Transcribed Defined Approach Requirement count does not match this file's own "
        "declared total_practices_in_source — check REQUIREMENTS for a missing or "
        "duplicated leaf item before shipping this data."
    )
