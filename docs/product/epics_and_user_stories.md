...
## Epic 5: Framework Mapping Engine — Delivered, retrieval-only (Sprint 5); cross-framework equivalence delivered, partial coverage (Sprint 10)
...
*Acceptance criteria:* `framework_mapping/cross_framework_equivalence.yaml` entries surface as an "Also satisfies" indicator in the evidence-link view. **Delivered, partial coverage, Sprint 10** — 79 of 106 NIST CSF 2.0 subcategories have a real, human-reviewed C2M2 equivalent (not an automatic similarity-threshold match, per `.claude/skills/framework-mapping/SKILL.md`'s explicit requirement); each entry carries a rationale sentence and its computed similarity, both shown together in the UI, never the score alone. The remaining 27 were reviewed and deliberately excluded — either a genuine standards gap (C2M2 has no equivalent practice) or because the only textually similar C2M2 practices were generic boilerplate repeated identically across all 10 domains, which would misrepresent a many-domains-match pattern as a specific finding. See ADR-0019.
...
## Epic 8: NERC CIP Framework Support — Started, partial coverage (Sprint 11)

**US-8.1** As Priya, I want the platform to know NERC CIP's actual standard/requirement/part structure (including per-part "Applicable Systems" scoping by BES Cyber System impact tier), so a NERC CIP assessment validates `practice_reference` against something real, the same way C2M2 and NIST CSF 2.0 already do.
*Acceptance criteria:* `framework_mapping/nerc_cip.yaml` encodes all 13 currently-mandatory standards (CIP-002 through CIP-014) with real, source-verified `purpose`/`source_version`/`source_url` metadata; `models/framework.py` gained `Practice.applicability` and `Domain.source_version`/`source_url` to represent NERC CIP's genuinely new structural concepts (per-part impact-tier scoping; a suite of independently-versioned standards rather than one document) — confirmed as a real schema mismatch against parsed source text before evolving the schema, not anticipated speculatively (ADR-0021). CIP-004-7 (Personnel & Training) is fully transcribed — 6 Requirements, 19 of 19 Parts, matching the source document's own structure exactly; the other 12 standards are honest structural stubs (`practices_populated: false`), not fabricated. Verified live: a NERC CIP assessment can be created in the frontend, evidence linked to a real `CIP-004-1.1` practice reference, and its "Applicable systems" text renders correctly in the Evidence tab.

**US-8.1a (surfaced by delivering US-8.1, not started)** As Priya, I want the remaining 12 NERC CIP standards transcribed with the same verbatim rigor as CIP-004, so a NERC CIP assessment can be meaningfully scored across the whole suite, not one standard of it.
*Acceptance criteria:* each standard's `practices_populated` flips to `true` via `backend/scripts/generate_nerc_cip_yaml.py`, following the process ADR-0021 and `.claude/skills/nerc-cip-expert/SKILL.md` document — real source PDF fetched (curl + browser User-Agent, since `WebFetch` is blocked by NERC's WAF) and parsed with `pypdf` before transcribing, same discipline as US-3.1a for C2M2. See risk R-26.
