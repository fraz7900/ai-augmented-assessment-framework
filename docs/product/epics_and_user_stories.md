...
## Epic 6: Executive Reporting — Dashboard delivered (Sprint 6), PDF/XLSX planned (Sprint 7)
...
*Acceptance criteria (draft):* Sprint 7 scope, per the `executive-reporting` skill's stated Sprint 6/7 split — report generation building on `DashboardReport`'s already-structured output, not a rewrite of the underlying computation.
