...
| PDF/XLSX report generation (Sprint 7) | Must | The dashboard (Sprint 6) is API-only; a CISO who needs to bring findings to a board without API access needs a downloadable artifact, per US-6.2 |
...
| PDF/XLSX report generation | 4 (Marcus, Diane, Priya — same reach as the dashboard, different artifact) | 2 (built on `DashboardReport`'s already-structured output, not new computation) | 75% | 1 | 6.0 |
