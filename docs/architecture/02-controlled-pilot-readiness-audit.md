...
|---|---|---|---|---|
| 1 | Framework engine | **PARTIAL → COMPLETE for this sprint's scope (ADR-0031, ADR-0053)** | Genuinely framework-agnostic: `models/framework.py`'s `Domain`/`Objective`/`Practice` hierarchy is generic; `services/scoring_service.py`/`services/report_service.py` dispatch on `FrameworkDefinition.scoring_model`, never on framework name/identity (confirmed: zero `if framework ==`-style branches in `services/`, `api/`, `ai/`). `services/framework_loader.py`'s `_KNOWN_FRAMEWORKS` registry is the one place a new framework is added, as data. No version pinning existed before this sprint (fixed, ADR-0031). **Closed in a same-sprint follow-up (ADR-0053)**: `FrameworkRegistry` now resolves and caches by `(name, version)`, not bare name — an assessment's pinned `framework_version` actually resolves its own schema even after a framework's "latest" changes (verified via a direct regression test simulating exactly that), not just recorded as a label. `GET /frameworks/{name}?version=` and `GET /frameworks/{name}/versions` (new) let a caller browse/discover specific versions; `POST /assessments` can pin an explicit version at creation. | No real framework in this project has more than one version yet, so the multi-version mechanism is proven via a synthetic fixture, not real production data (ADR-0053's Consequences). `cross_framework_equivalence.yaml` has no version concept — equivalence text always resolves against each framework's latest version, a disclosed simplification if an older pinned version's practice IDs ever diverge from latest (not yet exercised against real drift). No frontend UI lets a user select or browse framework versions yet. |
| 2 | Document ingestion | **PARTIAL → improved this sprint (ADR-0038, ADR-0041)** | PDF/DOCX/TXT/MD/XLSX/CSV parsed via `services/document_parsers.py`, with real malformed-file handling (`try/except` around every parser, never a 500 — `test_document_parsers.py`), a real 25MB size limit (`core/config.py:41`, `services/ingestion_service.py:48`), scanned-PDF detection (`_MIN_CHARS_PER_PAGE` heuristic), real magic-byte/binary-content sniffing before any parser runs, and a decompression-bomb ceiling (DOCX/XLSX ZIP-metadata pre-check plus a format-agnostic post-extraction text-size ceiling) — all fixed in ADR-0038. **Fixed this sprint (ADR-0041)**: XLSX/CSV support — spreadsheet evidence (asset inventories, invoice logs) can now be ingested; each row renders as a self-describing, citable line ("Row N: Col1: val1 \| Col2: val2"), sheet names become "# Heading" markers so the existing structure-aware chunker splits by sheet with no new chunking logic needed. | No OCR. No wall-clock parsing timeout — disclosed, deliberately not attempted (ADR-0038's Alternatives: needs subprocess-level isolation). `Settings.allowed_extensions` (`core/config.py:42`) remains dead config, never referenced (extension enforcement already happens via `_EXTENSION_TO_FILE_TYPE`, so this isn't a real gap, just an unused duplicate). **Closed in Sprint 19**: the setting was deleted rather than wired up. By then it had also drifted to omit XLSX/CSV, making it inert *and* wrong — a second list of supported formats can only ever drift from the one the parser actually enforces. "No OCR" was also reversed and delivered in Sprint 19 (ADR-0055). |
| 3 | Evidence representation/provenance | **PARTIAL → COMPLETE for this sprint's scope (ADR-0039, ADR-0041, ADR-0042)** | No single `Evidence` class exists; fields split across `EvidenceLink` (`models/assessment.py`, the assessment-facing DB row), `EvidenceChunk`/`SourceDocumentMetadata` (`models/schemas.py`, the ingestion-time shapes). Real structured location does exist at chunk granularity: `char_start`/`char_end`/`section_reference`/`document_id`/`chunking_strategy` persist through search and linking (`repositories/vector_repository.py:28-41`) — not "an anonymous vector chunk with no provenance." `content_hash` (SHA-256) exists at the document level. **Fixed this sprint**: a durable `Document` registry (ADR-0039) with explicit, human-declared `supersedes_document_id` and `GET /documents/{id}` surfacing both directions — closing "a re-upload gets a fresh, unlinked document_id". XLSX sheet/row provenance (ADR-0041, embedded in chunk text, not a separate field). **`page_number`/`parser_version` (ADR-0042)**: PDF page boundaries are computed at parse time from the exact same per-page text this project's own extraction already builds (not re-derived, can't drift out of sync) and carried through chunking into `EvidenceChunk.page_number`, migrated onto pre-existing local vector stores via LanceDB's own `add_columns` schema-evolution API (verified against a hand-built legacy-schema store, mirroring `assessment_repository.py`'s SQLite migration pattern for the same class of problem). `parser_version` reports the real, installed library version that parsed each document (e.g. `"pypdf==6.14.2"`, read via `importlib.metadata`, never hand-typed) — persisted on `Document`, surfaced on `IngestionResult` and `GET /documents/{id}`. | No dedicated `row_number`/`sheet_name` structured fields (ADR-0041's disclosed follow-up — currently embedded in chunk text only). **Closed in a same-sprint follow-up (ADR-0052)**: `EvidenceChunk.row_number`/`sheet_name` now exist as dedicated fields, computed at chunking time from real row/sheet boundaries the parser builds (mirroring `page_number`'s exact design), migrated onto pre-existing local vector stores the same way `page_number` was. A chunk spanning a PDF page boundary is attributed to its starting page only (fixed-window chunking doesn't respect page boundaries), disclosed as a simplification, not full multi-page provenance. No sensitivity metadata. **Closed in a same-sprint follow-up (ADR-0050)**: the evidence review screen (`EvidenceTab.tsx`) now flags a superseded document's evidence links (`GET /documents/{id}`, live-consumed by the frontend for the first time), and PDF/XLSX exports mark superseded citations `[SUPERSEDED]` (`EvidenceCitation.is_superseded`). **The Dashboard tab's own `GapItem.cited_evidence` rendering gap (deliberately left open by ADR-0050) was closed in a further same-sprint follow-up (ADR-0051)**: `CitedEvidenceList` now renders every citation's document ID, review status, and superseded flag directly under each gap. |
...
| 6 | Human review/override workflow | **COMPLETE** | Accept/edit/reject on AI-proposed evidence links, real state-machine enforcement (`services/assessment_service.py.review_evidence`), finalized-assessment immutability independently enforced on every mutation path (`link_evidence`/`review_evidence`/`propose_mappings`/`set_practice_finding`/`request_more_evidence`/`resolve_evidence_request`, each checking `AssessmentStatus.FINALIZED`), frontend AI/human badge (`EvidenceSourceBadge.tsx`, citing NFR-4 directly). Practice-level `SATISFIED`/`NOT_APPLICABLE`/etc. findings cover the N/A and re-judgment cases (ADR-0030); editing an AI-proposed link's `practice_reference` preserves the original via `EvidenceLink.original_practice_reference` (ADR-0030). **Fixed this sprint (ADR-0043)**: `EvidenceRequest` — a reviewer's explicit "please go get X" workflow action, distinct from (and coexistable with) any `PracticeFindingStatus` compliance judgment; create/list/resolve endpoints, wired into the dashboard UI (`EvidenceRequestControls.tsx`/`EvidenceRequestBadge.tsx`) with an open-request badge and inline resolve action per gap. | No dedicated worklist/dashboard page aggregating open requests across an assessment (or across assessments) for a contributor like Sam — `GET /assessments/{id}/evidence-requests` already provides the data such a page would consume; deferred as a disclosed follow-up (ADR-0043's Alternatives), not built this sprint. |
...
| 13 | Evaluation/testing | **PARTIAL → improved this sprint (ADR-0044)** | 215 backend tests (pre-sprint) across unit, "integration-flavored unit" (real committed YAML), and two real integration files (real SQLite/LanceDB/FastAPI `TestClient`); 13 frontend tests. Reasonable unit coverage per layer. **No single test chained the full pipeline** before this sprint — fixed via `backend/tests/test_golden_path_e2e.py` (Section C). Retrieval-evaluation (precision/recall) scaffolding delivered via `services/aqs_service.py` (ADR-0034). **Fixed this sprint (ADR-0044)**: failure-injection tests (`test_ingestion_service.py`) simulating an embedder/vector-store/document-registry failure mid-ingest — found and documented a real, disclosed gap: a document-registry write failure after a successful vector-store write leaves the chunks orphaned (functional for evidence linking, but absent from `GET /documents/{id}`), not fixed that sprint (see ADR-0044's Consequences). **Closed in a same-sprint follow-up (ADR-0046)**: a compensating delete (`VectorRepository.delete_chunks_for_document`) now runs on a registry-write failure, removing the just-written chunks before the original exception propagates — a residual, disclosed risk remains if the compensating delete itself also fails (covered by its own test, not assumed away). Performance-regression tests (`test_vector_repository.py`) as complexity-SCALING guards (small-vs-large-corpus latency ratio, not absolute wall-clock thresholds — this project's own WSL2/OneDrive environment is too noisy for those to be reliable) directly protecting the exact O(total corpus) regression class ADR-0033 already found and fixed once. | No AI-grounding tests (none needed today, no generative output to ground — relevant if/when ADR-0036's reasoner decision is ever revisited). **Closed in a same-sprint follow-up (ADR-0048)**: `.github/workflows/ci.yml` now runs the backend pytest suite and the frontend's real typecheck/build/test on every push and PR, live-verified via a local GitHub Actions runner. No lint gate (`ruff`/`oxlint`) was added — both are configured but never previously enforced, and `ruff` alone surfaces 482 pre-existing findings; fixing or gating on those is disclosed as a real, separate, still-open decision (ADR-0048's Rationale). |
| 14 | Scalability | **MISSING → measured this sprint** | `backend/scripts/benchmark_scalability.py` (new) measures the real app/SQLite/LanceDB/ONNX stack at 100 and 1,000 synthetic documents (ADR-0033). Real numbers, 100→1,000 docs: ingestion 6.4→9.4 docs/sec (scales fine); evidence-linking 207ms→1,428ms per link (**7x degradation, root-caused to a real bug and fixed** — `VectorRepository.chunks_for_document` was doing `table.to_pandas()`, an O(total corpus size) full-table load, on every call); dashboard reads 44ms→20ms p50 (no degradation); LanceDB storage ~1MB→53MB (linear, expected); peak RSS 994MB→3,848MB. | Single-query retrieval latency (95ms→~4s) and `propose-mappings` (53s→15+ min) also degraded sharply, but did **not** reproduce at anywhere near that magnitude in isolated testing — most likely this session's own loaded WSL2 environment, not a proven algorithmic defect; disclosed as an open, unresolved question rather than asserted either way (ADR-0033). `_open_existing_table()` re-opening the table on every call (~40-120ms/call, real but smaller) was also found — **fixed the same sprint (ADR-0037)**: a cached table handle with `checkout_latest()` for freshness, concurrent-write safety verified via a real reader/writer thread stress test before shipping (per this row's own stated requirement), not assumed. See Section F.6 below for the measured before/after numbers. Concurrent-read degradation persisted across every measurement but its exact magnitude is judged unreliable given the same environment noise. |
