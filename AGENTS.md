...
both bootstrap and CI install with `--require-hashes` (ADR-0081) — so "it passed in CI" names a
specific set of versions AND the bytes they were built from. The interpreters are declared once in
`.python-version` and `.nvmrc` (ADR-0080); bootstrap refuses a mismatch rather than warning. Regenerate deliberately with
