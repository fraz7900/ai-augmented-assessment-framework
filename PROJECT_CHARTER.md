...
- Multi-tenant authentication and role-based access control. **Partially reversed, Sprint 22 — the data-model half only.** Reopened by direct project-owner instruction, on the same grounds and by the same route as the OCR reversal below: R-39 had recorded since Sprint 21 that one organisation's evidence could be attached to another organisation's assessment through the product, which is a confidentiality failure rather than a missing convenience, and no amount of disclosure makes it safe to hand a second client an instance that can do it. What was built is an `Organization` that owns assessments and documents, enforced server-side and sealed into the finalized record. What was **not** built, and remains out of scope: authentication, role-based access control, per-user permissions, and cloud deployment. Anything able to reach the API directly can still name any organisation, so this is client separation *through the product*, not tenancy — the residual is recorded as R-40 rather than implied to be solved. See `docs/adr/ADR-0063-organization-scope.md`.
...
- **Near-term extension:** NERC CIP mapping, given its direct regulatory relevance to the same bulk electric system entities that use C2M2. **Delivered, Sprint 11** — all 13 currently-mandatory standards fully transcribed, 141 of 141 practices; see `docs/adr/ADR-0021-nerc-cip-roadmap-extension-start.md` and `docs/adr/ADR-0022-nerc-cip-full-transcription.md`. Cross-framework equivalence against C2M2 (ADR-0023), ISO 27001 (ADR-0024), CIS Controls (ADR-0025), SOC 2 (ADR-0026), PCI DSS (ADR-0027), and NIST CSF 2.0 (ADR-0028, Sprint 15 — the highest hit rate of any pairing, 107 of 141) delivered — 121 of 141 practices now have at least one reviewed equivalent. This completes every reviewed-framework pairing in this project's roadmap.
- **Framework breadth:** ISO 27001 **delivered, titles-only, Sprint 11** — the full standard is a paid, copyrighted publication with no free access (confirmed directly, not assumed), so this is real Annex A structure/control titles only, not full requirement text; see `docs/adr/ADR-0024-iso-27001-titles-only-and-equivalence.md`. CIS Controls v8 **delivered, full transcription, Sprint 12** — confirmed directly it is published under a Creative Commons Attribution-NonCommercial-No Derivatives 4.0 license, genuinely free unlike ISO 27001, so all 18 Controls/153 Safeguards were fully transcribed with complete official text; see `docs/adr/ADR-0025-cis-controls-full-transcription-and-equivalence.md`. SOC 2 **delivered, criterion-statement-only, Sprint 13** — confirmed directly that the AICPA's Trust Services Criteria is copyrighted, all-rights-reserved content despite being freely downloadable, so this is real criterion-statement structure only, not the fuller points-of-focus text; see `docs/adr/ADR-0026-soc2-statement-only-and-equivalence.md`. PCI DSS **delivered, Section-level statement-only, Sprint 14**, later **extended to full leaf-level transcription, Sprint 16** — confirmed directly it is copyrighted the same way as ISO 27001/SOC 2, and additionally is uniquely three hierarchy levels deep; Sprint 14 deliberately stopped at the Section level (63 of 63) given that depth, naming the finer ~205-item leaf level as disclosed future work, and Sprint 16 completed it (249 of 249 real leaf-level "Defined Approach Requirements", directly counted). The NERC CIP↔PCI DSS equivalence pairing (ADR-0027) was re-reviewed at the new leaf granularity rather than left pointing at ids that no longer exist — 60 of the original 80 NERC CIP practices retained a real leaf-level equivalent; see `docs/adr/ADR-0027-pci-dss-section-level-statement-only-and-equivalence.md` and `docs/adr/ADR-0029-pci-dss-leaf-level-extension-and-equivalence-re-review.md`. Every named framework-breadth item in this section is now delivered.
