...
**Also this sprint: a Docker Compose deployment stack** (`deployment/`, ADR-0017) — `docker compose up` builds and runs both services together, with Ollama defined but gated behind a Compose profile (`docker compose --profile ollama up`) rather than started by default, since nothing in the running application actually calls it (retrieval-only chat, ADR-0014, is what's used). This piece carries a disclosed gap: the authoring environment has no Docker installed and no passwordless `sudo` to install it, so it was statically verified (compose YAML validity, every environment variable cross-checked against the real `Settings` class, the backend's `pip install .` step replicated in an isolated venv) but not run end-to-end — tracked as risk R-24, with the specific remaining steps listed in `deployment/README.md`.
